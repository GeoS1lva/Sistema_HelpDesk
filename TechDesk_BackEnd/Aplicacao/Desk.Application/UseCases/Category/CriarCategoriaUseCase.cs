        ...
        public async Task<ResultModel> AdicionarSubCategorias(List<SubCategoriaCriar> subCategorias, int categoriaId)
        {
            ...
            foreach(var subCategoria in subCategorias)
            {
                if (categoria.SubCategorias.Any(x => x.Nome == subCategoria.Nome))
                    return ResultModel.Erro($"A Categoria {categoria.Nome} já possui essa subcategoria");

                var novaSubCategoria = new SubCategoria(subCategoria.Nome, subCategoria.Prioridade, subCategoria.SLA, categoria.Id);
